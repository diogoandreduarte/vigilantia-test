#!/usr/bin/env python3
"""
GetCompanyUrl — script independente da análise RGPD.
Recebe os dados de uma empresa em JSON (por parâmetro, sem ficheiros) e devolve
o site oficial da empresa, dados de contacto (NIF, email, morada) e
informação de DNS (nameservers) do domínio.

Uso:
    python scripts/get_company_url.py '{"company_name": "Empresa Lda", "country": "PT"}'

Usa pesquisa web (DuckDuckGo pois é gratis ao contrario do google que é preciso uma api key paga), o registo público de empresas Racius.com, e
consultas WHOIS diretas. Ver README_get_company_url.md para exemplos de uso
e de output.
...
def main():
    """Ponto de entrada da linha de comandos: lê o JSON passado como
    argumento, corre get_company_urls, e imprime o resultado em JSON
    formatado. Erros (JSON inválido, falha na pesquisa, etc.) são impressos
    no stderr e terminam o script com código de saída 1."""
    if len(sys.argv) != 2:
        print("Uso: python get_company_url.py '<json com dados da empresa>'", file=sys.stderr)
        sys.exit(1)
